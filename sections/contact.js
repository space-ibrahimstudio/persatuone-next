...
export function Contact({ sectionId }) {
  ...
  const [inputData, setInputData] = useState({ name: "", phone: "", email: "", subject: "", message: "" });
  const [errors, setErrors] = useState({ name: "", phone: "", email: "", subject: "", message: "" });

  const cleanInput = () => setInputData({ name: "", phone: "", email: "", subject: "", message: "" });
  const validateStep = () => {
    const newErrors = {};
    let isValid = true;
  ...
  const getAnalytics = () => ga.event({ action: "submit-form", params: { name: inputData.name, phone: inputData.phone, email: inputData.email } });
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (validateStep()) {
      getAnalytics();
      try {
        const response = await fetch("/api/sendmail", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(inputData) });
  ...
  return (
    <section id={sectionId} section-view-id={sectionId} className={styles.contact} style={{ backgroundImage: "url(/img/img-18.webp)" }}>
      <div className={styles.contactCard}>
        <div className={styles.aboutHeading} style={{ gap: "var(--pixel-30)" }}>
          <h1 className={styles.contactTitle}>Be Our Distributor in Your Country</h1>
          <p className={styles.contactDesc}>Please contact us to know more about our company, products, and sample request.</p>
          <div className={styles.contactOption}>
            <div style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "flex-start", alignSelf: "stretch", gap: "var(--pixel-10)" }}>
              <img style={{ position: "relative", height: "auto", width: "var(--pixel-18)" }} alt="phone" src="/svg/phone.svg" />
              <p className={styles.contactDesc} style={{ fontSize: "var(--font-xsmall)", fontWeight: "400" }}>
                +62 821 2227 9540
              </p>
            </div>
            <div style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "flex-start", alignSelf: "stretch", gap: "var(--pixel-10)" }}>
              <img style={{ position: "relative", height: "auto", width: "var(--pixel-18)" }} alt="phone" src="/svg/mail.svg" />
              <p className={styles.contactDesc} style={{ fontSize: "var(--font-xsmall)", fontWeight: "400" }}>
                sales@persatu.one | sales@cartenzcocoa.com
              </p>
            </div>
            <div style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "flex-start", alignSelf: "stretch", gap: "var(--pixel-10)" }}>
              <img style={{ position: "relative", height: "auto", width: "var(--pixel-18)" }} alt="phone" src="/svg/building.svg" />
              <p className={styles.contactDesc} style={{ fontSize: "var(--font-xsmall)", fontWeight: "400" }}>
                Ruko North Sorrento Square, Jalan Ir. Sukarno No.3, Pagedangan, Tangerang, Banten, Indonesia, 15332
              </p>
            </div>
          </div>
        </div>
