...
export function Gallery({ sectionId }) {
  ...
  const toggleExpand = () => setExpanded((prevExpand) => !prevExpand);
    ...
    <section id={sectionId} section-view-id={sectionId} className={styles.certifications}>
      ...
      <div className={styles.galleryBody}>
        <div className={styles.galleryTab}>
          {albums.map((album) => (
            <GalleryButton key={album.id} buttonText={album.name} onClick={() => handleAlbumClick(album)} isActive={selectedAlbum.id === album.id} />
          ))}
        </div>
        <div className={styles.galleryList}>
          {selectedAlbum.images.slice(0, displayLimit).map((image, index) => (
            <div className={styles.galleryImageWrap} key={index}>
              <img className={styles.galleryImage} src={`/img/albums/${image}.webp`} loading="lazy" alt={image} />
            </div>
          ))}
        </div>
        <div className={styles.galleryCta}>{selectedAlbum.images.length > 8 && <Button id={expanded ? "less-gallery" : "expand-gallery"} buttonText={expanded ? "Show Less" : "Display All Photos"} radius="md" onClick={toggleExpand} />}</div>
      </div>
    </section>
  );
}
